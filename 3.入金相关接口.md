...
|p2 |是|string |Bitake分配的商户号（UID）,可在BitakeApp->api配置或https://merchant.bitake.io网站上获取  |
|p3 |是|string | 商户订单号,由商户生成，同一个商户号下唯一  |
|p4 |否|string | 支付方式，可不传（不传时默认CNY）,具体值请参考支付方式列表，传CNY、VND、USD 以外的值时，需要咨询Bitake客服此支付方式是否开通  |
...
- 注：商户crm如果想按照自己平台的入金汇率给用户入金，  
    比如商户crm入金汇率为6.6  
    bitake的入金汇率为6.5（可通过bitake报价接口查出）  
    用户在crm入金100美金  
    这时候用户需要付的金额就为 100*6.6=660元人民币  
    只需要用 100*6.6/6.5=101.538462 (结果小数点后保留6位有效数字)作为bitake入金接口的p1(入金usdt数量)传过来，用户付款金额即为660人民币  
    回调的时候，商户crm根据回调时候的订单号，查出用户在crm入的金额为100美金，为用户入100美金  
    最终达到的效果是：  
    1.用户在crm入金100美金  
    2.用户付款660人民币  
    3.商户在bitake得到101.538462*（1-入金手续费）个USDT  
...
```
{
    "code":200,
    "data":{
        "price":6.46 //Bitake平台USDT报价（汇率）,即购买一个usdt对应法币的价格
    },
    "message":"success"
}
```
...
- 入金 直接返回订单数据，商户需要在自己的crm系统做收银台页面时，可以调用此接口，如果用Bitake提供的收银台，请用3.1入金接口
...
```
{
    ...
    "data":{
        ...
        "orderNo":"USDT1556487965324", //Bitake订单号
        "currency":"USDT", //入金币种
...
- 商户入金异步通知，需要商户自己开发回调接口，当订单完成时，由Bitake 调用此接口通知商户用户入金结果

**请求URL：** 
- `由商户通过Bitake APP 或者 登录Bitake商户网站（https://merchant.bitake.io）配置回调地址 `
...
success
```
- 商户在收到通知后返回{"code":200} 或者 success 字符串 ,否则Bitake 视为商户未成功收到通知，并采用以下重复策略通知策略继续通知：15s-3m-10m-30m-1h-2h-6h-15h
- 如果在通知策略时间内商户仍未返回成功码，Bitake则视为商户服务异常，放弃本笔订单通知，相关订单状态可在商户平台查询
...
```
{
    ...
    "data":{
        "amount":"1000.0", //入金金额
        "orderNo":"USDT1600328924378686589", //Bitake 交易流水号
